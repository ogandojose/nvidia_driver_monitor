...
// APICapture holds configuration for capturing API responses
type APICapture struct {
	Enabled   bool
	OutputDir string
}

// CaptureHTTPGet captures HTTP GET requests and saves responses to disk
func CaptureHTTPGet(url, outputDir string) (*http.Response, error) {
	...
	if err := saveAPIResponse(url, body, outputDir); err != nil {
...
func saveAPIResponse(url string, body []byte, outputDir string) error {
